...
public class MainFormController implements Initializable {
    ...
    @FXML
    void btnBackup_OnAction(ActionEvent actionEvent) throws IOException {
        try {
                ...
                Task<Void> task = new Task<Void>() {
                    ...
                    protected Void call() throws Exception {
                        ...
                        if (getPassword().length() > 0) {
                            commands = new String[]{"mysqldump", "-h", getHost(), "-u", getUser(),
                                    "-p" + getPassword(), "--port", getPort(), getDb(), "--result-file", file.getAbsolutePath() + ((file.getAbsolutePath().endsWith(".sql")) ? "" : ".sql")};
                        } else {
                            commands = new String[]{"mysqldump", "-h", getHost(), "-u", getUser(), "--port", getPort(),
    ...
    @FXML
    void btnRestore_OnAction(ActionEvent actionEvent) {
        try {
            ...
            if (file != null) {
                ...
                String[] commands;
                if (getUser().length() > 0) {
                    commands = new String[]{"mysql", "-h", getHost(), "-u", getUser(),
                            "-p" + getPassword(), "--port", getPort(), getDb(), "-e", "source " + file.getAbsolutePath()};
                } else {
                    commands = new String[]{"mysql", "-h", getHost(), "-u", getUser(), "--port", getPort(),
